...
export function AssignedBadge({ carrierId }: { carrierId: CarrierId | undefined }) {
  if (!carrierId) return <span className="assignBadge" style={{ background: "#999" }}>unassigned</span>;
...
export function RecruiterBadge({ names, recruiterColor }: { names: string[]; recruiterColor: (name: string) => string }) {
  if (!names.length) return <span className="assignBadge" style={{ background: "#999" }}>no recruiter</span>;
  return (
    <>
      {names.map((r) => (
